...
<form id="form_usuario" method="post">
    <div class="btn-toolbar">
        <button type ="submit" class="btn btn-primary" id="inserir" name="inserir" value="inserir">
            <i class="fa fa-plus-square fa-fw"></i> Instituição 
        </button>
    </div>
    <br/>
    <div class="well">
        <div class="table-responsive">
            <table id="tabelaInstituicoes" class="table table-striped" class="display" cellspacing="0" width="100%">
                <thead>       
                    <tr>
                        <th>ID</th>
                        <th>Descrição</th>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#tabelaInstituicoes').DataTable({
            "processing": true,
            "ajax": "instituicao/getInstituicoes/",
            "order": [[1, "asc"]],
            deferRender: true,
            "fnRowCallback": function (nRow, aData) {
                $(this).on("click", "#deletaInstituicao" + aData[0], function () {
                    if (!confirm("Deseja realmente excluir a Instituição selecionada?")) {
                        return false;
                    }
                });
            },
            "aoColumnDefs": [
                ...
                },
                {
                    "aTargets": [3],
                    "mRender": function (data, type, full) {
                        return '<button class="btn btn-danger" id="deletaInstituicao' + full[0] + '" type="submit" name="deletar" value= "' + full[0] + '">Deletar <i class="fa fa-trash-o"></i></button>';
                    }
                }
            ]
        });
