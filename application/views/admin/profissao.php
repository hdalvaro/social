...
<form id="form_usuario" method="post">
    <div class="btn-toolbar">
        <button type ="submit" class="btn btn-primary" id="inserir" name="inserir" value="inserir">
            <i class="fa fa-plus-square fa-fw"></i> Profissao 
        </button>
    </div>
    <br/>
    <div class="well">
        <div class="table-responsive">
            <table id="tabelaProfissoes" class="table table-striped" class="display" cellspacing="0" width="100%">
                <thead>       
                    <tr>
                        <th>ID</th>
                        <th>Descrição</th>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#tabelaProfissoes').DataTable({
            "processing": true,
            "ajax": "profissao/getProfissoes/",
            "order": [[1, "asc"]],
            deferRender: true,
            "fnRowCallback": function (nRow, aData) {
                $(this).on("click", "#deletaProfissao" + aData[0], function () {
                    if (!confirm("Deseja realmente excluir a profissão selecionada?")) {
                        return false;
                    }
                });
            },
            "aoColumnDefs": [
                ...
                },
                {
                    "aTargets": [3],
                    "mRender": function (data, type, full) {
                        return '<button class="btn btn-danger" id="deletaProfissao' + full[0] + '" type="submit" name="deletar" value= "' + full[0] + '">Deletar <i class="fa fa-trash-o"></i></button>';
                    }
                }
            ]
        });
